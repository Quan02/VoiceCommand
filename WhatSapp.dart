...
class WhatSapp extends StatefulWidget {
  ...
  const WhatSapp({Key? key, required this.durationpage, required this.speech}) : super(key: key);
  ...
  Future _checkAnswer({required String userSaid}) async
  {
    ...
    }else if(_situation == 1)
    {
      ...
      if (_userReply.toLowerCase().contains('yes')) {
        ...
        _confirmation = '';
        _userReply = '';
        _sendWhatsappMessage(phoneNumber: _phone, message: _message);
      } else if (_userReply.toLowerCase().contains('no')) {
        _situation -= 1;
        _answer = '';
        _confirmation = '';
...
_sendWhatsappMessage({required String phoneNumber, required String message}) async {
